...
SCRIPT_NAME="$0"
if [[ $# < 4 || $# > 5 ]]; then
  printf 'Usage: %s dataset_folder resp_flow_database new_length split [result_database_folder]\n' $SCRIPT_NAME
  exit
fi
if [ ! -d $1 ]; then
...
SHUFFLED_TRAIN_FILE=$DATA_BASE_FOLDER/$(printf "ucf101_train_rgb_len%02d_split%02d.txt" $LENGTH $SPLIT)
SHUFFLED_TEST_FILE=$DATA_BASE_FOLDER/$(printf "ucf101_val_rgb_len%02d_split%02d.txt" $LENGTH $SPLIT)
