...
SCRIPT_NAME="$0"
if [[ $# < 1 || $# > 4 ]]; then
  printf 'Usage: %s dataset_folder resp_flow_database length [result_database_folder]\n' $SCRIPT_NAME
  exit
fi
if [ ! -d $1 ]; then
...
SHUFFLED_TRAIN_FILE=$DATA_BASE_FOLDER/$(printf "hollywood2_train_flow_len%d_split%d.txt" $LENGTH $SPLIT)
SHUFFLED_TEST_FILE=$DATA_BASE_FOLDER/$(printf "hollywood2_val_flow_len%d_split%d.txt" $LENGTH $SPLIT)
