...
if [[ $# == 5 && ! -d $5 ]]; then
  echo 'Fourth argument must be directory'
  exit
fi
...
SHUFFLED_TRAIN_FILE=$DATA_BASE_FOLDER/$(printf "ucf101_train_flow_len%d_split%02d.txt" $LENGTH $SPLIT)
SHUFFLED_TEST_FILE=$DATA_BASE_FOLDER/$(printf "ucf101_val_flow_len%d_split%02d.txt" $LENGTH $SPLIT)
