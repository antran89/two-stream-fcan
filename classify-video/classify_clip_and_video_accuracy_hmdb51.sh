...
printf "Starting clip prediction for HMDB51 split %s\n" $SPLIT
printf "Feature folder: %s\n" $FEATURE_FOLDER
printf "Probability extention: %s\n" $PROB_EXTENSION

python $INSTALL/classify_clip_accuracy_hmdb51.py --feature_folder=$FEATURE_FOLDER --split=$SPLIT --prob_extension=$PROB_EXTENSION

printf "Starting video prediction for HMDB51 split %s\n" $SPLIT
printf "Feature folder: %s\n" $FEATURE_FOLDER
printf "Probability extention: %s\n" $PROB_EXTENSION

python $INSTALL/classify_video_accuracy_hmdb51.py --feature_folder=$FEATURE_FOLDER --split=$SPLIT --prob_extension=$PROB_EXTENSION
