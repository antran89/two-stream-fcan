...
printf "Starting clip prediction for UCF101 split %s\n" $SPLIT
printf "Feature folder: %s\n" $FEATURE_FOLDER
printf "Probability extention: %s\n" $PROB_EXTENSION

python $INSTALL/classify_clip_accuracy_ucf101.py --feature_folder=$FEATURE_FOLDER --split=$SPLIT --prob_extension=$PROB_EXTENSION

printf "Starting video prediction for UCF101 split %s\n" $SPLIT
printf "Feature folder: %s\n" $FEATURE_FOLDER
printf "Probability extention: %s\n" $PROB_EXTENSION

python $INSTALL/classify_video_accuracy_ucf101.py --feature_folder=$FEATURE_FOLDER --split=$SPLIT --prob_extension=$PROB_EXTENSION
