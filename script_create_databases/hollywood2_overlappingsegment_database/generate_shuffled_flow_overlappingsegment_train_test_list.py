'''
Creating Hollywood2 shuffled flow train test list.
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Creating Hollywood2 shuffled flow train test list.')
...
def main():
    ...
    for ind in xrange(len(actions)):
        action = actions[ind]
        action_split_file = '%s/%s_train.txt' % (info_folder, action)
        lines = np.loadtxt(action_split_file, dtype=str, delimiter='  ', comments=None)
        for line in lines:
           if int(line[1]) == 1:
               trainFiles.append(np.array([line[0], str(ind)]))
    for ind in xrange(len(actions)):
        action = actions[ind]
        ...
        for line in lines:
           if int(line[1]) == 1:
               testFiles.append(np.array([line[0], str(ind)]))

    numTrainFiles = len(trainFiles)
    numTestFiles = len(testFiles)
    #assert(numTrainFiles == 823)
